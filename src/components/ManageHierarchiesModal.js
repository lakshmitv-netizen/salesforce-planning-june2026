...
const imgErrorIcon = "data:image/svg+xml,%3Csvg width='16' height='16' viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='8' cy='8' r='7' fill='%23c23934'/%3E%3Cpath d='M5.5 5.5l5 5M10.5 5.5l-5 5' stroke='white' stroke-width='1.5' stroke-linecap='round'/%3E%3C/svg%3E";

const imgWarningIcon = "data:image/svg+xml,%3Csvg width='16' height='16' viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='8' cy='8' r='7' fill='%23f39c12'/%3E%3Cpath d='M8 4v5M8 11v1' stroke='white' stroke-width='1.5' stroke-linecap='round'/%3E%3C/svg%3E";

const imgLoadingIcon = "data:image/svg+xml,%3Csvg width='16' height='16' viewBox='0 0 16 16' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='8' cy='8' r='6' stroke='%23e5e5e5' stroke-width='2'/%3E%3Cpath d='M14 8a6 6 0 0 1-6 6' stroke='%230176d3' stroke-width='2' stroke-linecap='round'/%3E%3C/svg%3E";
...
export default function ManageHierarchiesModal({ isOpen, onClose, onNavigateToHierarchyPage, hierarchies: propHierarchies, setHierarchies: propSetHierarchies }) {
  ...
  const [syncingHierarchies, setSyncingHierarchies] = useState([]);
  const [isBulkSyncing, setIsBulkSyncing] = useState(false);

  if (!isOpen) return null;
  
  // Use the hierarchies from props
  const hierarchies = propHierarchies || [];
  const setHierarchies = propSetHierarchies || (() => {
    console.warn('setHierarchies function not provided');
  });
  ...
  const handleMenuAction = (action, hierarchy) => {
    ...
    if (action === 'edit') {
      ...
      setEditLevels([...(hierarchy.levels || [])]);
      ...
      // Use the hierarchy's actual levels if available, otherwise use defaults
      setCloneLevels(hierarchy.levels || [
        { id: 0, level: 0, name: 'Parent Account', isEditable: false },
        { id: 1, level: 1, name: 'Account', isEditable: false },
        { id: 2, level: 2, name: '', isEditable: true },
        { id: 3, level: 3, name: '', isEditable: true },
      ...
      setCloneNumLevels(hierarchy.numLevels || 5);
      ...
      // Start sync process
      setSyncingHierarchies(prev => [...prev, hierarchy.id]);
      
      // Update status to "Data Requested" immediately
      setHierarchies(prevHierarchies =>
        prevHierarchies.map(h =>
          h.id === hierarchy.id
            ? { ...h, dataStatus: 'Data Requested' }
            : h
        )
      );
      ...
      setTimeout(() => {
        const now = new Date();
        const formattedDate = `${String(now.getMonth() + 1).padStart(2, '0')}/${String(now.getDate()).padStart(2, '0')}/${now.getFullYear()}, ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')} ${now.getHours() >= 12 ? 'PM' : 'AM'}`;
        
        setHierarchies(prevHierarchies =>
          prevHierarchies.map(h =>
            h.id === hierarchy.id
              ? { ...h, dataStatus: 'Sync Successful', lastSync: formattedDate }
              : h
          )
        );
        
        setSyncingHierarchies(prev => prev.filter(id => id !== hierarchy.id));
        showSuccessToast('Hierarchy synced successfully');
      }, 2000);
    }
  };

  const getStatusIcon = (status, hierarchyId) => {
    if (syncingHierarchies.includes(hierarchyId)) {
      return imgLoadingIcon;
    }
  ...
  const toggleHierarchyActive = (hierarchy) => {
    const newActiveState = !hierarchy.isActive;
    
    setHierarchies(prevHierarchies => 
      prevHierarchies.map(h => {
        // If we're activating this hierarchy, deactivate all others in the same dimension
        if (newActiveState && h.dimension === hierarchy.dimension) {
          return h.id === hierarchy.id 
            ? { ...h, isActive: true }
            : { ...h, isActive: false };
        }
        // If we're deactivating this hierarchy, just deactivate it
        if (!newActiveState && h.id === hierarchy.id) {
  ...
  const toggleHierarchySelection = (hierarchyId) => {
    setHierarchies(prevHierarchies =>
      prevHierarchies.map(h =>
        h.id === hierarchyId ? { ...h, selected: !h.selected } : h
      )
    );
  };

  const toggleAllHierarchies = () => {
    const filteredHierarchies = selectedDimension === 'All' 
      ? hierarchies 
      : hierarchies.filter(h => h.dimension === selectedDimension);
    const allSelected = filteredHierarchies.every(h => h.selected);
    
    setHierarchies(prevHierarchies =>
      prevHierarchies.map(h => {
        const isInFiltered = filteredHierarchies.some(fh => fh.id === h.id);
        return isInFiltered ? { ...h, selected: !allSelected } : h;
      })
    );
  };

  const handleBulkSyncToDataCloud = async () => {
    const selectedHierarchies = hierarchies.filter(h => h.selected);
    ...
    setSyncingHierarchies(selectedHierarchies.map(h => h.id));
    ...
    setTimeout(() => {
      ...
      setSyncingHierarchies([]);
      showSuccessToast(
        `${selectedHierarchies.length} ${selectedHierarchies.length === 1 ? 'hierarchy' : 'hierarchies'} synced successfully`,
        'All selected hierarchies have been synced to Data Cloud'
      );
    }, 2000);
  };
  ...
  const openCreatePanel = () => {
    ...
    // Pre-fill dimension based on current filter selection
    if (selectedDimension === 'All') {
      setCreateDimension(''); // Empty string will trigger placeholder
    } else {
      setCreateDimension(selectedDimension); // 'Account' or 'Product'
    }
    setCreateLevels([
      { id: 0, level: 0, name: '', isEditable: true },
      { id: 1, level: 1, name: '', isEditable: true },
  ...
  const processAiResponse = (userInput) => {
    ...
    if (isCreating) {
      ...
      if (!isAccount && !isProduct && !input.includes('dimension')) {
        ...
        const dimension = isProduct ? 'Product' : 'Account';
...
What would you like to do?`;
  ...
  // Filter hierarchies based on selected dimension
  const filteredHierarchies = selectedDimension === 'All' 
    ? hierarchies 
    : hierarchies.filter(h => h.dimension === selectedDimension);
      ...
      <div className={`modal-container modal-hierarchies ${(editPanelOpen || clonePanelOpen || createPanelOpen || aiChatOpen) ? 'with-panel' : ''}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body-simple">
          ...
          <div className="hierarchies-header">
            <div className="hierarchies-info">
              <p className="hierarchies-label">Showing Available Hierarchies</p>
              <p className="hierarchies-count">
                {filteredHierarchies.length} {filteredHierarchies.length === 1 ? 'Hierarchy' : 'Hierarchies'} • 2 Dimensions
              </p>
            </div>
            <div className="hierarchies-button-group">
              <button className="hierarchies-sparkle-button" onClick={openAiChat}>
                <img src={imgSparkleIcon} alt="AI" />
              </button>
              <button 
                className="hierarchies-sync-button"
                onClick={handleBulkSyncToDataCloud}
                disabled={isBulkSyncing || !hierarchies.some(h => h.selected)}
                style={{
                  opacity: (isBulkSyncing || !hierarchies.some(h => h.selected)) ? 0.6 : 1,
                  cursor: (isBulkSyncing || !hierarchies.some(h => h.selected)) ? 'not-allowed' : 'pointer'
                }}
              >
                {isBulkSyncing ? 'Syncing...' : 'Sync to Data Cloud'}
              </button>
          ...
          <div className="scoped-notification">
            <span className="scoped-notification-text">
              ...
              <button 
                className="scoped-notification-link" 
                onClick={() => {
                  onClose();
                  if (onNavigateToHierarchyPage) {
                    onNavigateToHierarchyPage();
                  }
                }}
              >
          ...
          <div className="hierarchies-search-container">
            <div className="hierarchies-filter-group">
              <label className="hierarchies-field-label">Dimension</label>
              <select 
                className="hierarchies-filter-dropdown"
            ...
            <div className="hierarchies-search-group">
              <label className="hierarchies-field-label">Search</label>
          ...
          {/* Content Area: Table and Edit Panel side by side */}
          <div className="hierarchies-content-wrapper">
            {/* Hierarchies Table */}
                ...
                <thead>
                  <tr>
                    <th className="table-cell-checkbox">
                      <input 
                        type="checkbox" 
                        checked={filteredHierarchies.length > 0 && filteredHierarchies.every(h => h.selected)}
                        onChange={toggleAllHierarchies}
                      />
                    </th>
                    <th>Hierarchy</th>
                    <th>Dimension</th>
                  ...
                  {filteredHierarchies.map((hierarchy) => (
                    ...
                    >
                      <td className="table-cell-checkbox">
                        <input 
                          type="checkbox" 
                          checked={hierarchy.selected || false}
                          onChange={() => toggleHierarchySelection(hierarchy.id)}
                        />
                      </td>
                      ...
                      <td>{hierarchy.numLevels}</td>
                      <td>
                        <div className="data-status-cell">
                          <img 
                            src={getStatusIcon(hierarchy.dataStatus, hierarchy.id)} 
                            alt={hierarchy.dataStatus} 
                            className={`status-icon ${syncingHierarchies.includes(hierarchy.id) ? 'status-icon-spinning' : ''}`}
                          />
                          <span>{syncingHierarchies.includes(hierarchy.id) ? 'Syncing...' : hierarchy.dataStatus}</span>
                        </div>
                      </td>
                      <td>{hierarchy.lastSync}</td>
                        ...
                        <div className="dropdown-wrapper">
                          ...
                          {openMenuId === hierarchy.id && (
                            <div className="dropdown-menu">
                              ...
                              <label className="dropdown-menu-item dropdown-menu-checkbox-item">
                                <input 
                                  type="checkbox" 
                                  checked={hierarchy.isActive}
                                  onChange={() => toggleHierarchyActive(hierarchy)}
                                />
              ...
              <div className="edit-panel">
                ...
                <div className="edit-panel-header">
                  ...
                  <div className="edit-panel-header-actions">
                    ...
                    <button className="edit-panel-text-button edit-panel-save-button" onClick={() => {
                      // Update the hierarchy with new level names
                      const updatedHierarchy = {
                        ...selectedHierarchy,
                        levels: [...editLevels],
                        numLevels: editLevels.length
                      };
                      
                      // Update in hierarchies array
                      setHierarchies(prevHierarchies => 
                        prevHierarchies.map(h => h.id === selectedHierarchy.id ? updatedHierarchy : h)
                      );
                ...
                <div className="edit-panel-body">
                  ...
                  <div className="edit-panel-tree">
                    ...
                    {/* Tree Rows - dynamically rendered based on hierarchy levels */}
                    {editLevels && editLevels.map((level, index) => (
                      <div key={level.id} className={`tree-row${index > 0 ? ` tree-row-level-${index}` : ''}`}>
                        <div className="tree-cell">
                          <button className={`tree-chevron${index === editLevels.length - 1 ? ' tree-chevron-empty' : ''}`}>
                            {index < editLevels.length - 1 ? '▾' : ''}
                          </button>
                          <span className="tree-node-link">{selectedHierarchy.dimension} L{level.level}</span>
                        </div>
                        <div className="tree-cell">
                          <input 
                            type="text" 
                  ...
                  <div className="edit-panel-header-actions">
                    ...
                    <button className="edit-panel-text-button edit-panel-save-button" onClick={() => {
                      ...
                      const clonedHierarchy = {
                        ...
                        selected: false,
                        isActive: false,
                        numLevels: cloneLevels.length,
                        levels: [...cloneLevels]
                      };
                      
                      // Add to top of the list using functional update
                      setHierarchies(prevHierarchies => [clonedHierarchy, ...prevHierarchies]);
                        ...
                        return (
                          <div key={levelData.id} className={`clone-table-row ${indentClass}`}>
                            <div className="clone-table-cell">
                              ...
                              <span className="clone-level-text">{selectedHierarchy.dimension} L{levelData.level}</span>
                            </div>
                            <div className="clone-table-cell">
                              <input 
                                type="text" 
              ...
              <div className="edit-panel clone-panel">
                ...
                <div className="edit-panel-header">
                  ...
                  <div className="edit-panel-header-actions">
                    ...
                    <button className="edit-panel-text-button edit-panel-save-button" onClick={() => {
                      ...
                      const newHierarchy = {
                        id: `hier-${Date.now()}`,
                        name: createHierarchyName || 'New Hierarchy',
                        dimension: createDimension,
                        dataStatus: 'Not Synced',
                        lastSync: 'Never',
                        selected: false,
                        isActive: false,
                        numLevels: createLevels.length,
                        levels: [...createLevels]
                      };
                      
                      // Add to top of the list using functional update
                      setHierarchies(prevHierarchies => [newHierarchy, ...prevHierarchies]);
                ...
                <div className="edit-panel-body">
                  ...
                  <div className="clone-form-section">
                    ...
                    >
                      {createDimension === '' && <option value="">Select Dimension</option>}
                      <option value="Account">Account</option>
                      <option value="Product">Product</option>
        ...
        <div className="modal-footer">
          <button 
            ...
            onClick={onClose}
            disabled={editPanelOpen || clonePanelOpen || createPanelOpen || aiChatOpen}
          >
            Done
          </button>
        </div>
